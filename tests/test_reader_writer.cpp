...
IoUringFileReader RawReaderTest<IoUringFileReader>::create_reader() {
    return IoUringFileReader(test_file_path.string());
}
#endif
#endif
...
TEST(AsyncReaderTests, IoUringBasicAsyncRead) {
    ...
    Result<size_t> count = reader.wait_completions(completions, 1);
    ...
    EXPECT_EQ(std::memcmp(buffer.data(), test_data.data(), 1024), 0);
...
TEST(AsyncReaderTests, IoUringMultipleAsyncReads) {
    ...
    std::vector<std::pair<uint64_t, Result<std::size_t>>> completions;
    Result<size_t> count = reader.wait_completions(completions, 8);
    ASSERT_TRUE(count.is_ok());
...
TEST(AsyncReaderTests, IoUringPollCompletions) {
    ...
    while (poll_count++ < max_polls) {
        completions.clear();
        Result<size_t> count = reader.poll_completions(completions, 0);
...
TEST(AsyncReaderTests, IoUringWaitWithTimeout) {
    ...
    std::vector<std::pair<uint64_t, Result<std::size_t>>> completions;
    Result<size_t> count = reader.wait_completions_for(
        completions,
