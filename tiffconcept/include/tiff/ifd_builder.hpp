...
class IFDBuilder {
public:
    using OffsetType = typename ifd::IFDDescription<TiffFormat>::OffsetType;
    using TagType = typename ifd::IFDDescription<TiffFormat>::template TagType<TargetEndian>;
    using IFDType = ifd::IFD<TiffFormat, TargetEndian>;
...
public:
    IFDBuilder() = default;
    explicit IFDBuilder(IFDPlacement placement) : placement_strategy_(placement) {}
    ...
    /// Set the offset where external data will be written (call only before adding tags)
    void set_external_data_offset(std::size_t offset) noexcept {
        external_data_offset_ = offset;
    }
    
    /// Set the next IFD offset (for IFD chains)
    void set_next_ifd_offset(ifd::IFDOffset offset) noexcept {
        next_ifd_offset_ = offset;
    }
    
    /// Calculate total size needed for IFD (main array + external data)
    ...
    /// Get external data buffer (for writing separately)
