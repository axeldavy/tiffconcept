 ...
 *
 * - **IFD**: Complete IFD structure including all tags (except external arrays) and next IFD offset
 *
 * ## Example Usage
 *
 * @code{.cpp}
 ...
 */
struct IFDOffset {
    std::size_t value; //< Offset in the file of the IFD header
...
struct IFDDescription {
    IFDOffset offset; //< Offset in the file of the IFD header
    std::size_t num_entries; //< Number of tags in the IFD header
 ...
 *
 * This structure represents an extracted IFD including the inline tag entries
 * and the offset to the next IFD in the chain (if any).
 *
...
struct IFD {
    IFDDescription<TiffFormat> description; //< Header content
    std::vector<parsing::TagType<TiffFormat, SourceEndian>> tags; //< Inline tag array (unparsed and in raw format)
    IFDOffset next_ifd_offset; //< Offset to the next IFD if any
    ...
    [[nodiscard]] std::size_t num_tags() const noexcept; //< Number of tags in the IFD
    [[nodiscard]] constexpr static std::size_t size_in_bytes(std::size_t num_tags) noexcept; //< size for that format of header + tag array
    [[nodiscard]] std::size_t size_in_bytes() const noexcept; //< size of thus IFD
    [[nodiscard]] Result<void> write(std::span<std::byte> buffer) const noexcept; //< write the header and tag array is the target format.
    [[nodiscard]] std::vector<std::byte> write() const noexcept; //< Output the header and tag array in the raw format
};
