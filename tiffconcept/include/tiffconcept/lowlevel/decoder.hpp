...
/**
 * @file decoder.hpp
 * @brief Tile decoder for TIFF image data decompression and predictor decoding
 * 
 * This header provides the TileDecoder class for decoding compressed TIFF tiles
 * (tiles or strips). It handles both decompression and predictor decoding in a
 * single operation.
 * 
 * ## Key Concepts
 * 
 * - **Tile**: A tile (a strip is a tile) of image data that can be independently compressed
 * 
 ...
 * ## Thread Safety
 * 
 * TileDecoder is NOT thread-safe. Use one decoder instance per thread, or
...
/**
 * @brief Tile decoder for tiles and strips
 * 
 * TileDecoder handles the complete decoding pipeline for TIFF image tiles:
...
public:
    ...
    /**
     * @brief Decode tile into caller-provided buffer
     * 
     * Decodes compressed tile data directly into the provided output buffer.
     ...
     * 
     * Decodes compressed tile data into the decoder's internal scratch buffer
     ...
     * 
     * Decodes compressed tile data and returns a new vector containing the
